...
export default async function getTheaterMovies(payload) {
    const page = parseInt(payload[0]) || 1;
    const limit = 24;
    
    try {
        const filtered = movieData.filter(m => m.type === 'Movie'); // Giả định phim chiếu rạp là type Movie
        ...
        const mapMovie = (m) => ({
            ...
            year: m.release_year,
            time: m.duration || "N/A",
            episode_current: 'Phim Chiếu Rạp'
        });
        ...
        console.error("Lỗi khi lấy phim chiếu rạp:", err);
